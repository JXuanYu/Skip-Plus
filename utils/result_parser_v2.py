...
class ResultParser(object):
    ...
    def read_all(self):
        ...
        for dataset in prop['datasets']:
            for seed in prop['seeds']:
                ...
                acc, training_time, test_time, memory, parameter, _ = self._read_result(log_path)
    ...
    def read_base_to_new(self):
        ...
        for dataset in prop['datasets']:
            for seed in prop['seeds']:
                ...
                acc_base, training_time_base, test_time_base, memory, parameter, flops_base = self._read_result(base_log_path)
                acc_new, _, test_time_new, _, _, flops_new = self._read_result(new_log_path)
        ...
        cost_df = self._pivot(cost_df, ['training_time', 'base_test_time', 
                                        'new_test_time', 'memory', 'parameter', 'base_flops', 'new_flops'], prop)
    ...
    def _read_result(self, path):
        ...
        try:
            # FLOPs per sample: XX.XXX GFLOPs 或 Per-sample: XX.XXX GFLOPs
            flops_match = re.findall(r'Per-sample\: (\d+\.\d+) GFLOPs', content)
            if flops_match:
                flops = float(flops_match[-1])
            else:
                flops_match = re.findall(r'FLOPs per sample\: (\d+\.\d+) GFLOPs', content)
                flops = float(flops_match[-1]) if flops_match else 0.0
        except:
            flops = 0.0
        
        return acc, training_time, test_time, memory, parameter, flops
