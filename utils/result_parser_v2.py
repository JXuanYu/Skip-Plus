...
class ResultParser(object):
    ...
    def read_all(self):
        ...
        for dataset in prop['datasets']:
            for seed in prop['seeds']:
                ...
                stats = self._read_result(log_path)
                ...
                cost_dfs.append(pd.DataFrame({
                    ...
                    'test_time': [stats['test_time']],
                    'memory': [stats['memory']],
                    'parameter': [stats['parameter']],
                    'inference_flops': [stats['inference_flops']],
                    'train_flops': [stats['train_flops']],
                }))
                
        acc_df = pd.concat(acc_dfs, ignore_index=True)
        cost_df = pd.concat(cost_dfs, ignore_index=True)
        acc_df = self._pivot(acc_df, ['acc'], prop)
        cost_df = self._pivot(cost_df, [
            'training_time', 'test_time', 'memory', 'parameter',
            'inference_flops',
            'train_flops'
        ], prop)
    ...
    def read_base_to_new(self):
        ...
        for dataset in prop['datasets']:
            for seed in prop['seeds']:
                ...
                base_stats = self._read_result(base_log_path)
                new_stats = self._read_result(new_log_path)
                acc_base = base_stats['acc']
                acc_new = new_stats['acc']
                H = 2 / (1 / acc_base + 1 / acc_new) if acc_base > 0 and acc_new > 0 else 0
                acc_dfs.append(pd.DataFrame({
                    'dataset': [dataset],
                    'seed': [seed],
                    'base_acc': [acc_base],
                ...
                cost_dfs.append(pd.DataFrame({
                    'dataset': [dataset],
                    'seed': [seed],
                    'training_time': [base_stats['training_time']],
                    'base_test_time': [base_stats['test_time']],
                    'new_test_time': [new_stats['test_time']],
                    'memory': [base_stats['memory']],
                    'parameter': [base_stats['parameter']],
                    'base_flops': [base_stats['inference_flops']],
                    'new_flops': [new_stats['inference_flops']],
                    'train_flops': [base_stats['train_flops']],
                }))
        ...
        cost_df = self._pivot(cost_df, ['training_time', 'base_test_time', 
                                        'new_test_time', 'memory', 'parameter', 'base_flops', 'new_flops',
                                        'train_flops'], prop)

        acc_df = self._stat(acc_df, 'average', prop)
        for seed in prop['seeds']:
            # 避免除零错误
            base_acc_val = acc_df[f'base_acc_seed{seed}']
            new_acc_val = acc_df[f'new_acc_seed{seed}']
            acc_df[f'H_seed{seed}'] = 2 / (1 / base_acc_val + 1 / new_acc_val)
            acc_df[f'H_seed{seed}'] = acc_df[f'H_seed{seed}'].fillna(0)  # 处理NaN值
            
        acc_df['H'] = 2 / (1 / acc_df['base_acc'] + 1 / acc_df['new_acc'])
        acc_df['H'] = acc_df['H'].fillna(0)  # 处理NaN值
    ...
    def _read_result(self, path):
        ...
        try:
            # 推理 FLOPs（before_test 阶段）
            # FLOPs Analysis Complete. Per-sample: XX.XXX GFLOPs
            inference_flops_match = re.findall(r'FLOPs Analysis Complete\. Per-sample\: (\d+(?:\.\d+)?) GFLOPs', content)
            if inference_flops_match:
                inference_flops = float(inference_flops_match[-1])
            else:
                # 兼容旧日志格式
                inference_flops_match = re.findall(r'FLOPs per sample\: (\d+(?:\.\d+)?) GFLOPs', content)
                inference_flops = float(inference_flops_match[-1]) if inference_flops_match else 0.0
        except:
            inference_flops = 0.0

        try:
            # 训练 FLOPs（after_train，仅保留 Per-sample）
            train_flops_match = re.findall(
                r'Training FLOPs Analysis Complete(?: \(after_train\))?\. Per-sample: (\d+(?:\.\d+)?) GFLOPs',
                content
            )
            train_flops = float(train_flops_match[-1]) if train_flops_match else 0.0
        except:
            train_flops = 0.0
