...
if not hasattr(np, "int"):
    # Compatibility for old third-party augment code that still uses np.int.
    np.int = int
...
def extend_cfg(cfg):
    ...
    cfg.OPTIM.TOKEN_COMPRESS_LAYERS = ['prune_ddp.ratio_logit']
    cfg.OPTIM.TOKEN_COMPRESS_LR = 0.02

    cfg.OPTIM.CAT_LAYERS = ['film_img', 'film_text','cat_head']
    cfg.OPTIM.CAT_LR = 0.005

    # training configs
    cfg.TRAIN.CHECKPOINT_FREQ = -1
    cfg.TRAINER.PREC = 'fp16'

    # 训练 FLOPs 统计（在 after_train 进行）
    cfg.TRAINER.TRAIN_FLOPS = CN()
    cfg.TRAINER.TRAIN_FLOPS.ENABLE = True
    cfg.TRAINER.TRAIN_FLOPS.STEPS = 10

    # 推理 FLOPs 统计（在 before_test 使用真实测试 batch 进行）
    cfg.TRAINER.INFER_FLOPS = CN()
    cfg.TRAINER.INFER_FLOPS.ENABLE = True
    cfg.TRAINER.INFER_FLOPS.STEPS = 10
    cfg.TRAINER.INFER_FLOPS.WARMUP_STEPS = 1
    cfg.TRAINER.INFER_FLOPS.RUN_ONCE = True
    ...
    # loss settings
    cfg.TRAINER.SKIP.TOP_RATIO = 0.1
    cfg.TRAINER.SKIP.MAX_TOP = 50
    cfg.TRAINER.SKIP.LAMBDA = 0.7
    ...
    # ITM head weight
    cfg.TRAINER.SKIP.ITM_WEIGHT = 0.65

    # CAT Head Token 数量配置
    cfg.TRAINER.SKIP.CAT_KEEP_NUM = -1  # CAT Head 保留的 Token 数量
    # -1: 保留全部 tokens
    # 10: Top-10
    # 20: Top-20
    # 30: Top-30
    cfg.TRAINER.SKIP.EXP_MODE = "b2n"  # b2n | xd | all

    # 组件开关
    cfg.TRAINER.SKIP.USE_TSKIP = True
    cfg.TRAINER.SKIP.USE_DUAL_HEAD = True

    # === FLOPs损失 ===
    cfg.TRAINER.SKIP.USE_FLOPS_LOSS = True  # 启用FLOPs损失
    cfg.TRAINER.SKIP.TARGET_FLOPS = 15.0  # 目标FLOPs（GFLOPs）
    cfg.TRAINER.SKIP.FLOPS_LOSS_WEIGHT = 0.1  # FLOPs损失权重lambda
    cfg.TRAINER.SKIP.FLOPS_WARMUP_EPOCHS = 1  # Warm-up轮数

    # === Two-stage token compression training ===
    cfg.TRAINER.SKIP.TWO_STAGE_TRAINING = True
