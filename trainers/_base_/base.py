...
from utils.flops_counter import compute_skiptuning_inference_flops
...
class BaseTrainer(TrainerX):
    ...
    def before_test(self):
        self.perform_flops_analysis()

    def perform_flops_analysis(self):

        print("Starting FLOPs Analysis...")

        batch_size = getattr(self.cfg.DATALOADER.TEST, 'BATCH_SIZE', 1)
        image_size = getattr(self.cfg, 'INPUT', {}).get('SIZE', [224])[0] if hasattr(self.cfg, 'INPUT') else 224

        flops_results = compute_skiptuning_inference_flops(
            model=self.model,
            device=self.device,
