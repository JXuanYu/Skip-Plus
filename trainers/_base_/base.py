...
from utils.flops_counter import (
    compute_skiptuning_inference_flops_from_loader,
    compute_skiptuning_training_flops,
)
...
def load_clip_to_cpu_plus(cfg):
    """load SkipPlus-specific CLIP (with token-compression blocks)."""

    backbone_name = cfg.MODEL.BACKBONE.NAME
    url = clip_plus._MODELS[backbone_name]
    model_path = clip_plus._download(url)
    ...
    model = clip_plus.build_model(state_dict or model.state_dict())
    return model
...
class BaseTrainer(TrainerX):
    def __init__(self, cfg):
        super().__init__(cfg)
        self._inference_flops_profiled = False
    ...
    def after_train(self):
        self.perform_training_flops_analysis()
        super().after_train()
        # 仅在训练结束后进行一次模型统计
        self.model_summary()

    def perform_training_flops_analysis(self):
        cfg_flops = getattr(self.cfg.TRAINER, "TRAIN_FLOPS", None)
        enabled = getattr(cfg_flops, "ENABLE", False) if cfg_flops is not None else False
        num_steps = int(getattr(cfg_flops, "STEPS", 10)) if cfg_flops is not None else 10

        if not enabled or num_steps <= 0:
            return None

        flops_results = compute_skiptuning_training_flops(
            model=self.model,
            device=self.device,
            steps=num_steps,
            feature_loader=getattr(self, "feature_loader", None),
            train_loader=self.train_loader_x,
            parse_batch_train_fn=self.parse_batch_train,
        )
        if flops_results is None:
            print("Training FLOPs Analysis skipped: no available training batches.")
            return None

        print(
            "Training FLOPs Analysis Complete. "
            f"Per-sample: {flops_results['per_sample_gflops']:.3f} GFLOPs"
        )
        return flops_results

    def before_test(self):
        self.perform_inference_flops_analysis()

    def perform_inference_flops_analysis(self):
        cfg_flops = getattr(self.cfg.TRAINER, "INFER_FLOPS", None)
        enabled = getattr(cfg_flops, "ENABLE", True) if cfg_flops is not None else True
        run_once = getattr(cfg_flops, "RUN_ONCE", True) if cfg_flops is not None else True
        num_steps = int(getattr(cfg_flops, "STEPS", 10)) if cfg_flops is not None else 10
        warmup_steps = int(getattr(cfg_flops, "WARMUP_STEPS", 1)) if cfg_flops is not None else 1

        if not enabled or num_steps <= 0:
            return None

        if run_once and self._inference_flops_profiled:
            return None

        data_loader = self.test_loader if self.test_loader is not None else self.val_loader
        if data_loader is None:
            print("FLOPs Analysis skipped: no available test/val loader.")
            return None

        flops_results = compute_skiptuning_inference_flops_from_loader(
            model_inference_fn=self.model_inference,
            data_loader=data_loader,
            parse_batch_test_fn=self.parse_batch_test,
            device=self.device,
            steps=num_steps,
            warmup_steps=warmup_steps,
        )
        if flops_results is None:
            print("FLOPs Analysis skipped: insufficient test batches for profiling.")
            return None

        if "inference" in flops_results:
            inference_stats = flops_results["inference"]
            self.write_scalar("test/flops_per_sample", inference_stats["flops_per_sample"], self.epoch)
            self.write_scalar("test/gflops_per_sample", inference_stats["gflops_per_sample"], self.epoch)
            self.write_scalar("test/analysis_time", inference_stats["analysis_time"], self.epoch)
            print(f"FLOPs Analysis Complete. Per-sample: {inference_stats['gflops_per_sample']:.3f} GFLOPs")
            self._inference_flops_profiled = True
