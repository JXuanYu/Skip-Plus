...
class FeatureLoader(object):
    ...
    def get_features(self, epoch=None):
        ...
        batches = []
        for i, (feat_batch, label_batch) in enumerate(zip(features_batch, labels_batch)):
            batch_tuple = [feat_batch, label_batch]

            if len(self.zero_shot_features_all_epochs) > 0:
                zero_shot_batch = zero_shot_features[batch_idxs[i], :]
                batch_tuple.append(zero_shot_batch)

            batches.append(tuple(batch_tuple))
